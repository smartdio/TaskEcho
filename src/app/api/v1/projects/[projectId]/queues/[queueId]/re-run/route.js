/**
 * POST /api/v1/projects/:projectId/queues/:queueId/re-run
 * 重新运行队列：重置所有任务状态和拉取状态
 */
...
async function handlePOST(request, context) {
  try {
    ...
    updateFields['last_operation'] = 're-run';
    ...
    return createSuccessResponse({
      queue_id: queueId,
      rerun_tasks_count: tasks.length,
      rerun_at: now.toISOString()
    }, `成功重新运行队列，共 ${tasks.length} 个任务`);
    
  } catch (error) {
    console.error('重新运行队列失败:', error);
    return createErrorResponse(
